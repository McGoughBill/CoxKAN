from sumo.sumo.losses import total_loss, FastCoxLoss
...
def train_sumo(model,
			  ...
			  bs = 100, cuda = False,loss_func=total_loss):
	"""	
	...
	for i in t_bar:
		...
		for j in range(nbatches):
			...
			optimizer.zero_grad()
			if loss_func == total_loss:
				loss = loss_func(model,
								  xb,
								  tb,
								  eb)
			elif loss_func == FastCoxLoss:
				survival, intensity = model.forward(xb, tb, gradient=True)

				#make first column time, second column event in one tensor
				labels = torch.cat((tb.unsqueeze(1), eb.unsqueeze(1)), 1)
				loss = loss_func(survival,labels)
			loss.backward()
			optimizer.step()
		...
		if loss_func == total_loss:
			...
			#make first column time, second column event in one tensor
			labels = torch.cat((tb.unsqueeze(1), eb.unsqueeze(1)), 1)
			valid_loss = loss_func(survival,labels)
		else:
			raise NotImplementedError('Loss '+loss_func+' is not implemented')

		t_bar.set_description("Loss: {:.3f}".format(valid_loss.item()))
