...
datasets = {'gbsg': gbsg, 'metabric': metabric, 'flchain': flchain, 'support': support, 'nwtco': nwtco}
...
#load coxnnet config
with open(f'configs/coxnnet/{args.exp_name}.yml', 'r') as f:
    coxnnet_config = yaml.safe_load(f)

# load sumo config
with open(f'configs/sumo/{args.exp_name}.yml', 'r') as f:
    sumo_config = yaml.safe_load(f)
...
if coxnnet_config['early_stopping']:
...
mlp = coxnnet.CoxMLP(
    input_dim=len(covariates), hidden_dims=coxnnet_config['hidden_dim'],
    dropout=coxnnet_config['dropout'])
optimizer = tt.optim.Adam(lr=coxnnet_config['lr'], weight_decay=coxnnet_config['l2_reg'])
...
if coxnnet_config['early_stopping']:
    ...
    log = model.fit(
        X_train, y_train, batch_size=len(X_train), val_data=(X_val, y_val), epochs=coxnnet_config['steps'], verbose=False,
    ...
    log = model.fit(
        X_train, y_train, batch_size=len(X_train), epochs=coxnnet_config['steps'], verbose=False
    )

X_test = torch.tensor(df_test[covariates].values).double()
y_test = torch.tensor(df_test[[duration_col, event_col]].values).double()

def coxnnet_cindex(df):
    risk = - model.predict(torch.Tensor(df[covariates].values).double()).cpu().numpy()
    cindex = concordance_index(df[duration_col].astype(np.float32), risk.astype(np.float32),
                               df[event_col].astype(np.float32))
    return cindex

cindex_train = bootstrap_metric(coxnnet_cindex, df_train, N=100)['formatted']
cindex_test = bootstrap_metric(coxnnet_cindex, df_test, N=100)['formatted']

coxnnet_str = f'coxnnet - train: {cindex_train}, test: {cindex_test}';
print(coxnnet_str)
with open(output_dir / 'cindex.txt', 'a') as f:
    f.write(coxnnet_str + '\n')
results['coxnnet'] = {'train': cindex_train, 'test': cindex_test, 'params': coxnnet_config}


#### SUMO ####


if sumo_config['early_stopping']:
    train, val = train_test_split(df_train, test_size=0.2, random_state=42, stratify=df_train[event_col])
    X_val = val[covariates].values
    time_val = val[duration_col].values
    event_val = val[event_col].values
else:
    train = df_train
    val = None

X_train = df_train[covariates].values
time_train = df_train[duration_col].values
event_train = df_train[event_col].values
X_test = df_test[covariates].values
y_test = df_test[[duration_col, event_col]].values

model = sumo.SuMo(layers=sumo_config['init_params']['layers'], layers_surv=sumo_config['init_params']['layers_surv'],
                  dropout=sumo_config['init_params']['dropout'])
...
if sumo_config['early_stopping']:
    ...
    model.fit(X_train, time_train, event_train, val_data=(X_val, time_val, event_val),
              lr=sumo_config['optimizer_params']['lr'],
              weight_decay=sumo_config['optimizer_params']['weight_decay'], n_iter=sumo_config['n_iter'])
else:
...
def sumo_cindex(df):
    ...
    risk = model.predict_risk(X, time.tolist())
    risk = - np.mean(risk, axis=1)
    cindex = concordance_index(time, risk, df[event_col].values)
    print(cindex)
    return cindex

cindex_train = bootstrap_metric(sumo_cindex, df_train, N=100)['formatted']
cindex_test = bootstrap_metric(sumo_cindex, df_test, N=100)['formatted']
sumo_str = f'SuMo - train: {cindex_train}, test: {cindex_test}'; print(sumo_str)

with open(output_dir / 'cindex.txt', 'a') as f:
    f.write(sumo_str + '\n')
results['SuMo'] = {'train': cindex_train, 'test': cindex_test, 'params': sumo_config}
# save results
