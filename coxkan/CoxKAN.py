...
class CoxKAN(KAN):
    ...
    def train(self, df_train, df_val=None, duration_col='duration', event_col='event', covariates=None, 
        ...
        if early_stopping:
            ...
            save_loc = TEMP_CKPT_DIR / f'{best_model_hash}.pt'
            ...
            if df_val is not None:
                ...
                if early_stopping and step > 1:
                    if stop_on == 'cindex' and cindex_val > best_cindex:
                        ...
                        self.save_ckpt(save_loc, verbose=True)
        ...
        if early_stopping:
            print(save_loc)
            self.load_ckpt(save_loc, verbose=False)
    ...
    def suggest_symbolic(self, l, i, j, a_range=(-10, 10), b_range=(-10, 10), lib=None, topk=5, verbose=True,
                         interpretable=False,high_corr=0.9):
        ''' 
        ...
        if lib == None:
            ...
            if interpretable:
                lib = symbolic_lib = {key: symbolic_lib[key] for key in simplified_ordered_functions if key in SYMBOLIC_LIB}
        else:
            symbolic_lib = {}
            for item in lib:
        ...
        if interpretable:
            print('Using interpretable symbolic functions.')
            r2_scores = np.array(r2s)
            # Sort the attempted functions based on their predefined order in ordered_functions.
            sorted_indices = np.argsort([simplified_ordered_functions.index(item[0]) for item in lib_attempted])
            r2_scores = np.array(r2_scores)[sorted_indices]

            # see if we can use the softmax of the r2 scores to determine the best candidate
            if len(sorted_indices) > 0:
                ...
                if np.max(softmax_r2) > 0.5:
                    ...
                    return lib_attempted[sorted_indices[best_index]][0], \
                    lib_attempted[sorted_indices[best_index]][1], r2_scores[best_index]
                ...
                # otherwise, check if max of softmax is greater than 0.2 higher than second max - only viable if three or more functions contending.
                    ...
                    return lib_attempted[sorted_indices[best_index]][0], \
                    lib_attempted[sorted_indices[best_index]][
            ...
            if max(r2_scores) < high_corr:
                ...
                return lib_attempted[sorted_indices[best_index]][0], lib_attempted[sorted_indices[best_index]][
                    1], r2_scores[best_index]
            ...
            lib_attempted = [lib_attempted[i] for i in sorted_indices]

            # Re-sort for the remaining valid candidates.
            sorted_ids = np.argsort([simplified_ordered_functions.index(item[0]) for item in lib_attempted])
        else:
    ...
    def auto_symbolic(self, min_r2=0, a_range=(-10, 10), b_range=(-10, 10), lib=None, verbose=1,
                      only_interpretable_funcs=False):
        self.adjust_biases()
        '''
                ...
                for j in range(self.width[l + 1]):
                    if self.symbolic_fun[l].mask[j, i] > 0.:
                        ...
                        name, fn, r2 = self.suggest_symbolic(l, i, j, a_range=a_range, b_range=b_range, lib=lib, verbose=verbose > 1,
                                                            interpretable=only_interpretable_funcs)
                        if r2 >= min_r2:
